    ...
    public class PasswordResetController : ControllerBase
    {
        ...
        public IActionResult Put(PasswordResetModel model)
        {
            try
            {
                ...
                //capturar o email do usuário contido no TOKEN de autenticação
                var email = User.Identity.Name;

                //consultar o usuário no banco de dados
                var usuario = usuarioRepository.GetByEmailAndSenha(email, MD5Helper.Encrypt(model.SenhaAtual));
                ...
                if (usuario != null)
                {
                    ...
                    //HTTP 200 (OK)
                    return StatusCode(200, new { mensagem = "Senha de acesso atualizada com sucesso." });
                }
                else
                {
                    //HTTP 400 (BAD REQUEST)
                    return StatusCode(400, new { mensagem = "Senha atual inválida, por favor verifique." });
                }
            }
            catch (Exception e)
            {
