        ...
        public IActionResult Post(PasswordRecoverModel model)
        {
            try
            {
                ...
                if (usuario != null)
                {
                    ...
                    //HTTP 200 (OK)
                    return StatusCode(200, new { mensagem = "Nova senha gerada com sucesso, por favor verifique seu email." });
                }
                else
                {
                    //HTTP 400 (BAD REQUEST)
                    return StatusCode(400, new { mensagem = "Email inválido. Usuário não encontrado." });
                }
            }
            catch (Exception e)
            {
